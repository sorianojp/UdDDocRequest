...
export default function TrackResult({ request }: { request: DocumentRequest }) {
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            ...
            case 'WAITING_FOR_PAYMENT': return 'bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300';
            case 'VERIFYING_PAYMENT': return 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300';
            case 'PROCESSING': return 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300';
            case 'DEFICIENT': return 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300';
            case 'READY': return 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300';
            case 'CLAIMED': return 'bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-200';
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-background flex flex-col justify-center py-12 sm:px-6 lg:px-8">
            ...
            <div className="sm:mx-auto sm:w-full sm:max-w-6xl">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-6">
                        <Card className={
                            request.status === 'PENDING' ? 'border-gray-500 border-2 dark:border-gray-700' : 
                            request.status === 'WAITING_FOR_PAYMENT' ? 'border-yellow-500 border-2 dark:border-yellow-700' : 
                            request.status === 'VERIFYING_PAYMENT' ? 'border-orange-500 border-2 dark:border-orange-700' : 
                            request.status === 'DEFICIENT' ? 'border-red-500 border-2 dark:border-red-700' : 
                            request.status === 'READY' ? 'border-green-500 border-2 dark:border-green-700' : 
                            request.status === 'CLAIMED' ? 'border-slate-500 border-2 dark:border-slate-700' : 
                            request.status === 'REJECTED' ? 'border-red-500 border-2 dark:border-red-700' : 
                            request.status === 'PROCESSING' ? 'border-blue-500 border-2 dark:border-blue-700' : ''
                        }>
                            <CardHeader>
                                        ...
                                        <table className="min-w-full divide-y divide-border">
                                            <thead className="bg-muted/50">
                                                <tr>
                                                    ...
                                                    <th scope="col" className="px-4 py-3 text-right text-xs font-medium text-muted-foreground uppercase tracking-wider">Price</th>
                                                </tr>
                                            </thead>
                                ...
                                {request.deficiency_remarks && (
                                    <div className="bg-yellow-50 dark:bg-yellow-900/20 border-l-4 border-yellow-400 dark:border-yellow-600 p-4">
                                        <div className="flex">
                                            ...
                                            <div className="ml-3">
                                                <h3 className="text-sm font-medium text-yellow-800 dark:text-yellow-200">Deficiency Remarks</h3>
                    ...
                    <div className="space-y-6">
                        <Card className="bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800 border-2 shadow-sm">
                            <CardContent className="pt-6">
                                <div className="text-center space-y-4">
                                    <div className="space-y-1">
                                        <h3 className="text-sm font-medium text-blue-800 dark:text-blue-300 uppercase tracking-wider">Reference Number</h3>
                                        <p className="text-3xl sm:text-4xl font-mono font-bold text-blue-900 dark:text-blue-100 tracking-widest select-all">
                                            {request.reference_number}
                                        </p>
                                    </div>
                                    <div className="flex items-center justify-center gap-2 text-sm text-blue-800 dark:text-blue-200 bg-blue-100/50 dark:bg-blue-800/50 p-3 rounded-md mx-auto max-w-md">
                                        <div className="bg-blue-200 dark:bg-blue-700 p-1 rounded-full">
                                            <Info className="w-4 h-4 text-blue-800 dark:text-blue-100" />
                                        </div>
                            ...
                            <CardContent>
                                {request.payment && request.status !== 'WAITING_FOR_PAYMENT' ? (
                                    <div className="space-y-4">
                                        <div className="p-4 rounded-md border text-center bg-muted/50 border-border">
                                            {request.payment.reference_number ? (
                                                ...
                                                <p className="text-muted-foreground italic">No reference number</p>
                                            )}
                                        </div>
                                        ...
                                        {request.payment.proof_file_path && (
                                            <div>
                                                ...
                                                <div className="mt-2 border rounded-md p-2 bg-muted/50 border-border">
                                    ...
                                    <div className="rounded-md">
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            <div className="bg-red-50 dark:bg-red-900/20 p-4 rounded-md border border-red-100 dark:border-red-900">
                                                <h4 className="font-semibold text-red-900 dark:text-red-200 mb-2">BPI</h4>
                                            ...
                                            <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-md border border-green-100 dark:border-green-900">
                                                <h4 className="font-semibold text-green-900 dark:text-green-200 mb-2">LAND BANK</h4>
                                        ...
                                        {isPaymentRejected && (
                                            <div className="bg-red-50 dark:bg-red-900/20 p-3 rounded-md border border-red-200 dark:border-red-800 flex items-center gap-2 text-red-800 dark:text-red-300 text-sm mb-2 mt-4">
                                        ...
                                        <form onSubmit={submitPayment} className="space-y-4 pt-2">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <Label 
                                                    ...
                                                    className="flex flex-col items-center justify-center p-4 border-2 border-dashed border-border rounded-md hover:bg-accent transition-colors cursor-pointer bg-card"
                                                >
                                                    {preview ? (
                                        ...
                                        <div className="bg-amber-50 dark:bg-amber-900/20 p-6 rounded-md border border-amber-200 dark:border-amber-800 text-center space-y-3">
                                            <Clock className="h-12 w-12 text-amber-500 mx-auto" />
                                            <h4 className="text-lg font-medium text-amber-900 dark:text-amber-200">Waiting for Verification</h4>
            ...
            <div className="mt-8 border-t pt-8 pb-4">
                <div className="text-center space-y-2">
                    <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider">Registrar's Office Contact Information</h3>
                    <div className="text-sm text-muted-foreground">
                        <p className="mt-1">
                            Email: <a href="mailto:registrar@udd.edu.ph" className="text-primary hover:underline">registrar@udd.edu.ph</a>
                        </p>
                        <p>
